...
public class GroupCreated implements MessageCommand{
    ...
    public Predicate<Message> messagePredicate() {
        return message -> (message.getChat().getType().equals("group") || message.getChat().getType().equals("supergroup")) && message.getNewChatMembers().get(0).getUserName().equals(botUsername);
    }
